...
// Database connection - mimics the one in integrations/database_connection.js and backend/src/db.js
const pool = new Pool({
  ...
  try {
    ...
    for (const cat of serviceCategories) {
      const result = await client.query(
        `INSERT INTO categorias_servico (nome) 
         VALUES ($1) 
    ...
    // 2. Ensure measurement units exist
    ...
    const itServices = [
      ...
      { nome: 'Instalacao de Leitor de Cartoes', categoria: 'Upgrade e Melhorias', tempo: 20, valor: 30.00, comissao: 10.00, descricao: 'Instalacao interna de leitor de cartoes SD/MicroSD na frente do gabinetes', cuidados_antes: 'Verificar disponibilidade de bay de 3,5\" ou 5,25\"', cuidados_depois: 'Conectar cabos USB interno ao header da placa mae' },
    ...
    for (const service of itServices) {
      // Check if service already exists by nome
    ...
    const itProducts = [
      ...
      // Gabinetes
      { codigo: 'GAB001', desc: 'Gabinete Cooler Master MasterBox Q300L Micro-ATX', grupo: 'Gabinetes', unidade: 'UN', precoCusto: 180.00, margem: 25.00, estoque: 6, estoqueMin: 2 },
    ...
    for (const product of itProducts) {
      ...
      // Check if product already exists by codigo_barras
      if (product.codigo) {
      ...
      // Also check by descricao as fallback
